...
def load_deliverables_table() -> str:
    """Read deliverables.md and return the full deliverables table section."""
    try:
        text = DELIVERABLES.read_text()
        # Extract everything from the Full Deliverables Table heading onwards
        marker = "## Full Deliverables Table"
        idx = text.find(marker)
        if idx == -1:
            return "*[deliverables table not found — check deliverables/deliverables.md]*"
        # Stop before the Immediate priorities section
        end_marker = "## Immediate priorities"
        end_idx = text.find(end_marker, idx)
        section = text[idx:end_idx].strip() if end_idx != -1 else text[idx:].strip()
        return section
    except FileNotFoundError:
        return "*[deliverables/deliverables.md not found]*"
...
def main():
    ...
    content = CONTENT.replace("{DELIVERABLES_TABLE}", load_deliverables_table())
