...
def build_md():
    ...
    p()
    row(f"| | n | Sensitivity | Specificity | Precision | F1 | κ vs gold | Human κ |")
    row(f"|---|---|---|---|---|---|---|---|")
    row(f"| **Cochrane / O'Mara-Eves target** | — | **≥ 0.95** | — | — | — | **≥ 0.60** | — |")
    row(f"| **Human screeners** (Hanegraaf et al. 2024) | — | — | — | — | — | — | 0.82 (abstract) / 0.77 (full-text) |")
    row(f"| **AI — GPT-4** (Zhan et al. 2025) | — | 0.992 | 0.836 | — | — | 0.83 | — |")
    row(f"| **AI mean, 172 studies** (Scherbakov et al. 2025) | — | 0.804 | — | 0.632 | 0.708§ | — | — |")
    row(f"| | | | | | | | |")
    ...
    row(f"| | | | | | | | |")
    row(f"| **Benchmark reached? (R2a)** | | ⚠ Near miss | ✓ Yes | ✓ Yes | ~ No target | ⚠ Near miss | ✓ Yes |")
    row(f"| **Notes** | | 0.897 < 0.95; above 172-study mean (0.804); conservative defaults raise effective sensitivity | Exceeds GPT-4 (0.836) | Exceeds 172-study mean (0.632) | No T/A screening F1 benchmark reported; our 0.839 exceeds Scherbakov computed 0.708 | Above min. threshold (0.60); below human 0.82 | Meets threshold |")
    p()
    p("†R3a: designed to verify criteria stability, not generate a new gold standard. P/R/F1/specificity not computable. LLM κ is mean of κ vs Jennifer Cisse (0.690) and κ vs Caroline Staub (0.674).")
    p("§Scherbakov et al. 2025: F1 computed from reported sensitivity (0.804) and precision (0.632) — not directly reported in the paper.")
    p()
    p("**Reading the table:** Two benchmarks were not met at R2a — sensitivity (0.897 vs ≥0.95) and LLM κ (0.770 vs human 0.82). Both are near misses: κ improved by 0.334 points across three rounds through criteria revision alone, and the conservative inclusion default raises effective sensitivity above the calibration figure. Specificity (0.905) and precision (0.788) both exceed available benchmarks. F1 (0.839) has no published T/A screening target but exceeds the only computable peer figure (Scherbakov 0.708).")
    p()
