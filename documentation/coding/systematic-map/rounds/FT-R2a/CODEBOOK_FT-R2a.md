...
These papers were randomly sampled from the full-text retrieval pool — **not all will qualify**. Read each paper and apply the five criteria genuinely. Set `confirmed_include = yes` only if the paper meets **all five**. If it fails any criterion, set `confirmed_include = no` and note the reason in `notes`.
