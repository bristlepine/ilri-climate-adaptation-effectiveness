# Systematic Map Codebook — FT-R2a
...
**[→ Open the FT-R2a folder on Google Drive](https://drive.google.com/drive/folders/1yS0liSnb8prB4m6MkTe8XgCp6sIZ3tCz?usp=drive_link)**

| What | Where |
|------|-------|
| Papers to code | `FT-R2a pdfs/` subfolder inside the Drive folder above |
| Blank coding sheet | `coding_ft-r2a_template.csv` inside the Drive folder |
| Your completed sheet | Save as `coding_ft-r2a_INITIALS.csv` (e.g. `coding_ft-r2a_AZ.csv`) and upload back to the same folder |

---

## How to code

1. Download `coding_ft-r2a_template.csv` and save a copy as `coding_ft-r2a_INITIALS.csv`
2. Open each paper from the `FT-R2a pdfs/` subfolder
3. Apply the five inclusion criteria (see below) — enter `confirmed_include` first
4. If `confirmed_include = yes`: fill in all 16 extraction fields for that row
5. If `confirmed_include = no`: leave extraction fields blank; note the reason in `notes`
6. Enter your initials in `coder_id` — same initials on every row
7. Upload your completed file back to the Drive folder when done

**Rules:**

- Code independently — do not discuss your decisions with anyone until instructed
...
These papers have been retrieved but not yet formally screened. Read each paper and confirm it meets **all five criteria** before extracting data. The paper must satisfy ALL five — if it fails any one, mark `confirmed_include = no`.
...
**Decision rule:** Enter `yes` in `confirmed_include` if the paper meets ALL five criteria. Enter `no` if it fails any one criterion — leave extraction fields blank and briefly note which criterion failed in the `notes` column.
...
Read the paper in full, then apply the five inclusion criteria from the cover page.

| Value | Meaning |
|-------|---------|
| `yes` | Paper meets all five criteria — proceed to fill in fields 1–16 |
| `no` | Paper fails one or more criteria — leave fields 1–16 blank; record reason in `notes` |
