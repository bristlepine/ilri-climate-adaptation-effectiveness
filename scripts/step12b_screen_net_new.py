"""
step12b_screen_net_new.py

Abstract screening for all net-new records from additional databases
(output of step2b_multidatabase_dedupe.py: WoS, CAB Abstracts, AGRIS, ASP).
...
Caching: decisions are stored in step12b_results_details.jsonl keyed by
record hash. Re-runs skip already-screened records, so adding a new database
to step2b and re-running this script only screens the new records.

Outputs (under outputs/step12b/):
  - step12b_results.csv           — screening decisions for all net-new records
...
Run: python scripts/step12b_screen_net_new.py
"""
